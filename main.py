...
def predict_heatmap():
    ...
    try:
        ...
        # FIX 1: Use 'dimensions: 256' instead of 'scale'. 
        # This FORCES Earth Engine to return a small footprint image regardless of how big the polygon is.
        # It guarantees the download is instant and PyTorch never runs out of RAM on Render.
        url = input_image.getDownloadURL({'dimensions': 256, 'crs': 'EPSG:4326', 'region': roi, 'format': 'GEO_TIFF'})
        
        # Added a timeout so the request doesn't hang Gunicorn indefinitely
        ...
        # FIX 2: Bulletproof NaN & Infinity removal
        # np.isfinite checks for both NaNs and Infs and safely replaces them with 0
        band0 = np.where(np.isfinite(padded[0]), padded[0], 0)
        safe_lulc = band0.astype(int)
        lulc_onehot = np.eye(9)[np.clip(safe_lulc, 0, 8)]
        
        band1 = np.where(np.isfinite(padded[1]), padded[1], 0)
        dist_band = np.expand_dims(np.clip(band1/5000.0, 0, 1), -1)
