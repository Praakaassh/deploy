...
def predict_heatmap():
    ...
    try:
        ...
        # FIX 1: Increased scale to 100 to prevent Render Out-Of-Memory (OOM) crashes
        url = input_image.getDownloadURL({'scale': 100, 'crs': 'EPSG:4326', 'region': roi, 'format': 'GEO_TIFF'})
        ...
        # FIX 2: Safely handle NaN values generated by out-of-bounds clipping from Earth Engine
        safe_lulc = np.nan_to_num(padded[0], nan=0).astype(int)
        lulc_onehot = np.eye(9)[np.clip(safe_lulc, 0, 8)]
        
        safe_dist = np.nan_to_num(padded[1], nan=0)
        dist_band = np.expand_dims(np.clip(safe_dist/5000.0, 0, 1), -1)
